...
class HomeFrontController extends Controller
{
    ...
    function sitemap()
    {
        ...
        $title = (isset($_COOKIE['language']) && $_COOKIE['language'] == '2')
            ? 'साइटमैप'
            : 'Sitemap';
        $quickLinks = DB::table('quick_links')->where('is_deleted', 0)->where('status', 1)->when(
            in_array($language, [1, 2]),
            fn($q) =>
            $q->where('language', $language)
        )->get();
        $footerLinks = DB::table('menus')->where('txtpostion', 3)->where('menu_status', 1)->when(
            in_array($language, [1, 2]),
            fn($q) =>
            $q->where('language', $language)
        )->get();
