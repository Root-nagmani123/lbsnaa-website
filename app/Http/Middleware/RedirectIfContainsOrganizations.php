...
class RedirectIfContainsOrganizations
{
    public function handle(Request $request, Closure $next)
    {
        // Define a regex pattern for the desired keywords
        $pattern = '/(organization(s?)|organization[_-]?setup|setup[_-]?organization)/';

        // Check if the URL path matches the defined pattern
        if (preg_match($pattern, $request->path())) {
            ...
            // If the URL matches the pattern and has a slug parameter
            if ($request->is('lbsnaa-subs/micromenu/*') && $request->has('slug')) {
                $slug = $request->query('slug', 'default-slug'); // Default slug if not available
            ...
            // Handle specific case for URLs like 'lbsnaa-sub_org/organizations/*'
            if ($request->is('lbsnaa-sub_org/organizations/*')) {
                return $next($request); // No redirection needed
            }
        }
