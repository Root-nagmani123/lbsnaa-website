                                ...
                                @foreach ($records as $record)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @if ($record->id != 1)
                                                <form action="{{ route('organisation_chart.destroy', $record->id) }}"
                                                    method="POST" style="display:inline;">
                                                    @csrf
                                                    <button type="submit"
                                                        class="btn btn-sm btn-primary text-white">Delete</button>
                                                </form>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="form-check form-switch">
                                                <input class="form-check-input status-toggle" type="checkbox" role="switch"
                                                    data-table="organisation_chart" data-column="status"
                                                    data-id="{{ $record->id }}" {{ $record->status ? 'checked' : '' }}>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
