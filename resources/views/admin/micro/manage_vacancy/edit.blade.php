    ...
    <form action="{{ route('manage_vacancy.update', $vacancy->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mb-4">
                ...
                <div class="form-group position-relative">
                    <select class="form-select form-control ps-5 h-58" name="research_centre" id="research_centre" required>
                        <option value="" disabled {{ is_null($vacancy->research_centre) ? 'selected' : '' }}>
                            Select Research Centre
                        </option>
                        @foreach($researchCentres as $id => $name)
                            <option value="{{ $id }}" {{ (string)$vacancy ->research_centre === (string)$id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
		...
		<div class="form-group" id="document_upload" style="display: {{ old('content_type', $vacancy->content_type) == 'PDF' ? 'block' : 'none' }};">
            ...
            @if ($vacancy->document_upload)
                <!-- Display the current document if it exists -->
